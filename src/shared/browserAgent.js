    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          selector: {
            type: ['string', 'null'],
            description: 'CSS selector for the element to click. Omit or pass null if using text.',
          },
          text: {
            type: ['string', 'null'],
            description:
        ...
        properties: {
          records: {
            ...
            items: {
              type: 'object',
              properties: {
                ownerName: { type: ['string', 'null'], description: 'Full name of the property owner/taxpayer' },
                propertyAddress: { type: ['string', 'null'], description: 'Full property/mailing address' },
                parcelId: { type: ['string', 'null'], description: 'Parcel ID, account number, or property ID' },
                taxYear: { type: ['string', 'null'], description: 'Tax year (e.g. "2025")' },
                taxAmountDue: { type: ['string', 'null'], description: 'Total tax amount due (e.g. "$1,234.56")' },
                paymentStatus: {
                  type: ['string', 'null'],
                  description: 'Payment status: Paid, Unpaid, Partial, Delinquent, etc.',
                },
                county: { type: ['string', 'null'], description: 'County name' },
                state: { type: ['string', 'null'], description: 'State abbreviation (e.g. "TX")' },
                legalDescription: { type: ['string', 'null'], description: 'Legal description of the property' },
                additionalDetails: {
                  type: 'object',
          ...
          totalFound: { type: ['number', 'null'], description: 'Total count of matching records found' },
          summary: {
            type: ['string', 'null'],
            description: 'One-sentence summary of what was found (e.g. "Found 2 records for John Smith")',
          },
          searchedUrl: { type: ['string', 'null'], description: 'The URL where results were found' },
        },
...
async function executeTool(page, toolName, rawInput) {
  // Strip null values — Groq (Llama 4) sometimes sends optional fields as null
  // even when the schema uses type:['string','null']. Remove them so downstream
  // code can safely use truthiness checks (if input.selector) etc.
  const input = Object.fromEntries(
    Object.entries(rawInput || {}).filter(([, v]) => v !== null && v !== undefined)
  );
